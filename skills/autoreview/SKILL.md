...
- Tracked WebP payloads are intentionally omitted from source review only after RIFF structure checks and successful decoding with trusted `dwebp` from the external PATH. Their path, MIME type, change status, and old/new byte sizes remain in the bundle. Review the accompanying text diff normally and validate the opaque assets separately. Missing libwebp tools, invalid WebPs, oversized assets, and other tracked binary types still fail closed.
...
- validates tracked WebPs with external `dwebp`, then reports them as explicit metadata-only records instead of aborting the surrounding source review; missing decoders, invalid assets, and unsupported binary types still fail closed
