...
- Reviewer processes have a 30-minute absolute deadline by default. Set `--engine-timeout-seconds` or `AUTOREVIEW_ENGINE_TIMEOUT_SECONDS` to a larger positive value before starting a review that is expected to need longer.
- Treat heartbeat lines like `review still running: ... elapsed=... pid=...` as healthy progress within that deadline, not a hang. Heartbeats do not extend the wall-clock limit. Pass `--stream-engine-output` when live engine text is useful; Codex and Claude filter tool/file chatter, other runnable engines pass raw output through.
- Do not manually kill a review just because it has been quiet for 2-5 minutes or remains active inside its configured deadline. Inspect the process only after missing multiple expected heartbeats or after an obviously failed subprocess; prefer letting the same helper command finish or report its bounded timeout.
...
| `AUTOREVIEW_ENGINE_TIMEOUT_SECONDS` | Positive wall-clock limit for each reviewer process; defaults to 1800 seconds                                                    |
