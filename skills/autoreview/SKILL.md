...
- Review bundles omit security-sensitive paths or files, but autoreview does not secret-scan or rewrite code patches. Repositories should enforce credential detection with a deterministic pre-commit hook and CI scanner such as TruffleHog; install and run the repository-required hook instead of changing legitimate code to satisfy autoreview. Explicit prompt and dataset inputs remain checked before engine invocation. Safe large diffs are sent as one pass while they fit the aggregate prompt limit, then partitioned into complete bounded passes without truncation.
